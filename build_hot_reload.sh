...
odin build "$SRC_DIR" -extra-linker-flags:"$EXTRA_LINKER_FLAGS" -define:RAYLIB_SHARED=true -build-mode:dll -out:$OUT_DIR/game_tmp$DLL_EXT -strict-style -vet -debug
...
echo "Building $EXE"
odin build "$SRC_DIR/main_hot_reload" -out:$EXE -strict-style -vet -debug
